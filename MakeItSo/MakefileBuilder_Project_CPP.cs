    ...
    class MakefileBuilder_Project_CPP
    {
        ...
        private MakefileBuilder_Project_CPP(ProjectInfo_CPP project)
        {
            ...
            try
            {
                ...
                m_file.WriteLine(String.Format("-include {0}.mkdefs", m_projectInfo.Name));
                m_file.WriteLine("");
        ...
        private void createIncludePathVariables()
        {
            ...
            foreach (ProjectConfigurationInfo_CPP configuration in m_projectInfo.getConfigurationInfos())
            {
                ...
                {
                    string realPath = MakeItSoConfig.Instance.getPath(path);
                    includePath += String.Format("-I{0} ", Utils.quote(realPath));
                }
        ...
        private void createLibraryPathVariables()
        {
            ...
            foreach (ProjectConfigurationInfo_CPP configuration in m_projectInfo.getConfigurationInfos())
            {
                ...
                {
                    string realPath = MakeItSoConfig.Instance.getPath(path);
                    libraryPath += String.Format("-L{0} ", Utils.quote(realPath));
                }
        ...
        private void createLibrariesVariables()
        {
            ...
            foreach (ProjectConfigurationInfo_CPP configuration in m_projectInfo.getConfigurationInfos())
            {
                ...
                {
                    string shortLibName = libraryName.StartsWith("lib") ? libraryName.Substring(3) : libraryName;
                    libraries += String.Format("-l{0} ", shortLibName);
                }
        ...
        private void createConfigurationTarget(ProjectConfigurationInfo_CPP configurationInfo)
        {
            ...
            string objectFiles = "";
            foreach (string filePath in m_projectInfo.getFiles())
            {
                string path = Utils.computeTargetPath(filePath, intermediateFolder);
                path = MakeItSoConfig.Instance.getPath(path);
            ...
            string targetName = configurationInfo.TargetName.Length > 0 ? configurationInfo.TargetName : m_projectInfo.Name;
            string outputPath;
            string buildCmd;
            switch (m_projectInfo.ProjectType)
            {
                ...
                case ProjectInfo_CPP.ProjectTypeEnum.CPP_EXECUTABLE:
                    outputPath = String.Format("{0}/{1}", outputFolder, targetName);
                    ...
                    buildCmd = String.Format("\tg++ {0} $({1}) $({2}) -Wl,-rpath,{3} -o {3}/{4}",
                        objectFiles, libraryPath, libraries, outputFolder, targetName);
                ...
                case ProjectInfo_CPP.ProjectTypeEnum.CPP_STATIC_LIBRARY:
                    ...
                    outputPath = String.Format("{0}/lib{1}.a", outputFolder, targetName);
                    buildCmd = String.Format("\tar rcs {0}/lib{1}.a {2} {3}", outputFolder, targetName, objectFiles, implicitlyLinkedObjectFiles);
                    break;
                ...
                case ProjectInfo_CPP.ProjectTypeEnum.CPP_DLL:
                    ...
                    outputPath = String.Format("{0}/{1}", outputFolder, targetName);
                    buildCmd = String.Format("\tg++ {0} -shared -Wl,-soname,{1} -o {2}/{1} {3} {4} $({5}) $({6})",
                        pic, targetName, outputFolder, objectFiles, implicitlyLinkedObjectFiles,
                        dllLibraryPath, dllLibraries);
                    break;
            ...
            m_file.WriteLine("{0}: {1} {2}", configurationInfo.Name, dependencies, outputPath);
            m_file.WriteLine("{0}: {1}", outputPath, objectFiles);
            m_file.WriteLine(buildCmd);
        ...
        private void createFileTargets(ProjectConfigurationInfo_CPP configurationInfo)
        {
            ...
            foreach (string filePath in m_projectInfo.getFiles())
            {
                ...
                string path = Utils.computeTargetPath(filePath, intermediateFolder);
                string realFilePath = MakeItSoConfig.Instance.getPath(filePath);
                path = MakeItSoConfig.Instance.getPath(path);
                string objectPath = Path.ChangeExtension(path, ".o");
                ...
                m_file.WriteLine("# Compiles file {0} for the {1} configuration...", realFilePath, configurationInfo.Name);
                m_file.WriteLine("-include {0}", dependenciesPath);
                m_file.WriteLine("{0}: {1}", objectPath, realFilePath);
                m_file.WriteLine("\t{0} {1} {2} -c {3} {4} -MMD -o {5}", compiler, preprocessorDefinitions, compilerFlags, realFilePath, includePath, objectPath);
                // m_file.WriteLine("\t{0} {1} {2} -c {3} {4} -o {5}", compiler, preprocessorDefinitions, compilerFlags, realFilePath, includePath, objectPath);
                // m_file.WriteLine("\t{0} {1} {2} -MM {3} {4} > {5}", compiler, preprocessorDefinitions, compilerFlags, realFilePath, includePath, dependenciesPath);
                m_file.WriteLine("");
            }
        }
        ...
        private void createCreateFoldersTarget()
        {
            ...
            foreach (ProjectConfigurationInfo_CPP configuration in m_projectInfo.getConfigurationInfos())
            {
                ...
                intermediateFolder = MakeItSoConfig.Instance.getPath(intermediateFolder);
                string outputFolder = getOutputFolder(configuration);
                outputFolder = MakeItSoConfig.Instance.getPath(outputFolder);
        ...
        private string getIntermediateFolder(ProjectConfigurationInfo_CPP configuration)
        {
            ...
            string folder = Utils.addPrefixToFolderPath(configuration.IntermediateFolder, prefix);
            return MakeItSoConfig.Instance.getPath(folder);
        }
        ...
        {
            string prefix = MakeItSoConfig.Instance.getProjectConfig(m_projectInfo.Name).CPPFolderPrefix;
            string folder = Utils.addPrefixToFolderPath(configuration.OutputFolder, prefix);
            return MakeItSoConfig.Instance.getPath(folder);
        }
