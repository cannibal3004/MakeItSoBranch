...
namespace MakeItSo
{
    ...
    class MakefileBuilder_Project_CSharp
    {
        ...
        private void createConfigurationFlagsVariable(ProjectConfigurationInfo_CSharp configurationInfo)
        {
            ...
            // Type of debug info...
            if (configurationInfo.DebugInfo != "" && configurationInfo.DebugInfo != "none")
            {
        ...
        private void createReferencesVariables()
        {
            ...
            foreach (ProjectConfigurationInfo_CSharp configurationInfo in m_projectInfo.getConfigurationInfos())
            {
                ...
                {
                    value += (TrimReferenceFileName(getReferenceRelativePath(referenceInfo)) + ",");
                }
                value = value.TrimEnd(',');
                // value = "";
                if (value != "-r:") m_file.WriteLine(variableName + " = " + value);
            }
        }

        private static string TrimReferenceFileName(string referencePath)
        {
            var index = referencePath.LastIndexOf("/", StringComparison.Ordinal);
            var refName = referencePath.Substring(index + 1, referencePath.Length - index - 1);
            //var length = referencePath.Length;
            //Log.log("reference added: " + refName);
            return FixCaseSensitiveFileNames(refName);
            
        }

        private static string FixCaseSensitiveFileNames(string fileName)
        {
            var charArray = fileName.ToCharArray();
            var lastDot = fileName.LastIndexOf(".", StringComparison.Ordinal);
            charArray[0] = Char.ToUpper(charArray[0]);
            for (int i = 1; i < lastDot; i++)
            {
                if (charArray[i] == "."[0])
                {
